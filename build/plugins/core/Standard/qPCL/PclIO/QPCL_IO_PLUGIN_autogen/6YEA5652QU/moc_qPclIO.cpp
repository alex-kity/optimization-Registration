/****************************************************************************
** Meta object code from reading C++ file 'qPclIO.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../../../../../plugins/core/Standard/qPCL/PclIO/include/qPclIO.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#include <QtCore/qplugin.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'qPclIO.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_qPclIO_t {
    QByteArrayData data[1];
    char stringdata0[7];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_qPclIO_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_qPclIO_t qt_meta_stringdata_qPclIO = {
    {
QT_MOC_LITERAL(0, 0, 6) // "qPclIO"

    },
    "qPclIO"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_qPclIO[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       0,    0, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

       0        // eod
};

void qPclIO::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    Q_UNUSED(_o);
    Q_UNUSED(_id);
    Q_UNUSED(_c);
    Q_UNUSED(_a);
}

const QMetaObject qPclIO::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_qPclIO.data,
      qt_meta_data_qPclIO,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *qPclIO::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *qPclIO::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_qPclIO.stringdata0))
        return static_cast<void*>(const_cast< qPclIO*>(this));
    if (!strcmp(_clname, "ccIOPluginInterface"))
        return static_cast< ccIOPluginInterface*>(const_cast< qPclIO*>(this));
    if (!strcmp(_clname, "cccorp.cloudcompare.ccPluginInterface/3.2"))
        return static_cast< ccPluginInterface*>(const_cast< qPclIO*>(this));
    if (!strcmp(_clname, "cccorp.cloudcompare.ccIOPluginInterface/1.3"))
        return static_cast< ccIOPluginInterface*>(const_cast< qPclIO*>(this));
    return QObject::qt_metacast(_clname);
}

int qPclIO::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    return _id;
}

QT_PLUGIN_METADATA_SECTION const uint qt_section_alignment_dummy = 42;

#ifdef QT_NO_DEBUG

QT_PLUGIN_METADATA_SECTION
static const unsigned char qt_pluginMetaData[] = {
    'Q', 'T', 'M', 'E', 'T', 'A', 'D', 'A', 'T', 'A', ' ', ' ',
    'q',  'b',  'j',  's',  0x01, 0x00, 0x00, 0x00,
    '0',  0x02, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00,
    0x1c, 0x02, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x03, 0x00, 'I',  'I',  'D',  0x00, 0x00, 0x00,
    '!',  0x00, 'c',  'c',  'c',  'o',  'r',  'p', 
    '.',  'c',  'l',  'o',  'u',  'd',  'c',  'o', 
    'm',  'p',  'a',  'r',  'e',  '.',  'p',  'l', 
    'u',  'g',  'i',  'n',  '.',  'q',  'P',  'c', 
    'l',  'I',  'O',  0x00, 0x9b, 0x09, 0x00, 0x00,
    0x09, 0x00, 'c',  'l',  'a',  's',  's',  'N', 
    'a',  'm',  'e',  0x00, 0x06, 0x00, 'q',  'P', 
    'c',  'l',  'I',  'O',  ':',  0xa0, 0xa0, 0x00,
    0x07, 0x00, 'v',  'e',  'r',  's',  'i',  'o', 
    'n',  0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00,
    0x05, 0x00, 'd',  'e',  'b',  'u',  'g',  0x00,
    0x15, 0x10, 0x00, 0x00, 0x08, 0x00, 'M',  'e', 
    't',  'a',  'D',  'a',  't',  'a',  0x00, 0x00,
    0x9c, 0x01, 0x00, 0x00, 0x0d, 0x00, 0x00, 0x00,
    0x84, 0x01, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x04, 0x00, 't',  'y',  'p',  'e',  0x00, 0x00,
    0x03, 0x00, 'I',  '/',  'O',  0x00, 0x00, 0x00,
    0x9b, 0x05, 0x00, 0x00, 0x04, 0x00, 'n',  'a', 
    'm',  'e',  0x00, 0x00, 0x0c, 0x00, 'P',  'C', 
    'D',  ' ',  'f',  'i',  'l',  'e',  ' ',  'I', 
    '/',  'O',  0x00, 0x00, 0x1b, 0x0a, 0x00, 0x00,
    0x0b, 0x00, 'd',  'e',  's',  'c',  'r',  'i', 
    'p',  't',  'i',  'o',  'n',  0x00, 0x00, 0x00,
    0x1d, 0x00, 'O',  'p',  'e',  'n',  ' ',  'P', 
    'C',  'D',  ' ',  'f',  'i',  'l',  'e',  's', 
    ' ',  '(',  'P',  'C',  'L',  ' ',  'l',  'i', 
    'b',  'r',  'a',  'r',  'y',  ')',  '.',  0x00,
    0x14, 0x10, 0x00, 0x00, 0x07, 0x00, 'a',  'u', 
    't',  'h',  'o',  'r',  's',  0x00, 0x00, 0x00,
    '<',  0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
    '8',  0x00, 0x00, 0x00, ',',  0x00, 0x00, 0x00,
    0x03, 0x00, 0x00, 0x00, '(',  0x00, 0x00, 0x00,
    0x1b, 0x03, 0x00, 0x00, 0x04, 0x00, 'n',  'a', 
    'm',  'e',  0x00, 0x00, 0x0b, 0x00, 'L',  'u', 
    'c',  'a',  ' ',  'P',  'e',  'n',  'a',  's', 
    'a',  0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    0x85, 0x01, 0x00, 0x00, 0x14, 0x1a, 0x00, 0x00,
    0x0b, 0x00, 'm',  'a',  'i',  'n',  't',  'a', 
    'i',  'n',  'e',  'r',  's',  0x00, 0x00, 0x00,
    '<',  0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
    '8',  0x00, 0x00, 0x00, ',',  0x00, 0x00, 0x00,
    0x03, 0x00, 0x00, 0x00, '(',  0x00, 0x00, 0x00,
    0x1b, 0x03, 0x00, 0x00, 0x04, 0x00, 'n',  'a', 
    'm',  'e',  0x00, 0x00, 0x0b, 0x00, 'L',  'u', 
    'c',  'a',  ' ',  'P',  'e',  'n',  'a',  's', 
    'a',  0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    0x85, 0x01, 0x00, 0x00, 0x94, '#',  0x00, 0x00,
    0x0a, 0x00, 'r',  'e',  'f',  'e',  'r',  'e', 
    'n',  'c',  'e',  's',  'h',  0x00, 0x00, 0x00,
    0x02, 0x00, 0x00, 0x00, 'd',  0x00, 0x00, 0x00,
    'X',  0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00,
    'P',  0x00, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x04, 0x00, 't',  'e',  'x',  't',  0x00, 0x00,
    0x0b, 0x00, 'P',  'C',  'L',  ' ',  'l',  'i', 
    'b',  'r',  'a',  'r',  'y',  0x00, 0x00, 0x00,
    0x9b, 0x06, 0x00, 0x00, 0x03, 0x00, 'u',  'r', 
    'l',  0x00, 0x00, 0x00, 0x17, 0x00, 'h',  't', 
    't',  'p',  ':',  '/',  '/',  'p',  'o',  'i', 
    'n',  't',  'c',  'l',  'o',  'u',  'd',  's', 
    '.',  'o',  'r',  'g',  '/',  0x00, 0x00, 0x00,
    0x0c, 0x00, 0x00, 0x00, '(',  0x00, 0x00, 0x00,
    0x85, 0x01, 0x00, 0x00, 'p',  0x00, 0x00, 0x00,
    '<',  0x00, 0x00, 0x00, 0xbc, 0x00, 0x00, 0x00,
    ' ',  0x00, 0x00, 0x00, 0x0c, 0x01, 0x00, 0x00,
    0x0c, 0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    'p',  0x00, 0x00, 0x00, '<',  0x00, 0x00, 0x00,
    'd',  0x00, 0x00, 0x00, 'T',  0x00, 0x00, 0x00
};

#else // QT_NO_DEBUG

QT_PLUGIN_METADATA_SECTION
static const unsigned char qt_pluginMetaData[] = {
    'Q', 'T', 'M', 'E', 'T', 'A', 'D', 'A', 'T', 'A', ' ', ' ',
    'q',  'b',  'j',  's',  0x01, 0x00, 0x00, 0x00,
    '0',  0x02, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00,
    0x1c, 0x02, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x03, 0x00, 'I',  'I',  'D',  0x00, 0x00, 0x00,
    '!',  0x00, 'c',  'c',  'c',  'o',  'r',  'p', 
    '.',  'c',  'l',  'o',  'u',  'd',  'c',  'o', 
    'm',  'p',  'a',  'r',  'e',  '.',  'p',  'l', 
    'u',  'g',  'i',  'n',  '.',  'q',  'P',  'c', 
    'l',  'I',  'O',  0x00, 0x95, 0x09, 0x00, 0x00,
    0x08, 0x00, 'M',  'e',  't',  'a',  'D',  'a', 
    't',  'a',  0x00, 0x00, 0x9c, 0x01, 0x00, 0x00,
    0x0d, 0x00, 0x00, 0x00, 0x84, 0x01, 0x00, 0x00,
    0x1b, 0x03, 0x00, 0x00, 0x04, 0x00, 't',  'y', 
    'p',  'e',  0x00, 0x00, 0x03, 0x00, 'I',  '/', 
    'O',  0x00, 0x00, 0x00, 0x9b, 0x05, 0x00, 0x00,
    0x04, 0x00, 'n',  'a',  'm',  'e',  0x00, 0x00,
    0x0c, 0x00, 'P',  'C',  'D',  ' ',  'f',  'i', 
    'l',  'e',  ' ',  'I',  '/',  'O',  0x00, 0x00,
    0x1b, 0x0a, 0x00, 0x00, 0x0b, 0x00, 'd',  'e', 
    's',  'c',  'r',  'i',  'p',  't',  'i',  'o', 
    'n',  0x00, 0x00, 0x00, 0x1d, 0x00, 'O',  'p', 
    'e',  'n',  ' ',  'P',  'C',  'D',  ' ',  'f', 
    'i',  'l',  'e',  's',  ' ',  '(',  'P',  'C', 
    'L',  ' ',  'l',  'i',  'b',  'r',  'a',  'r', 
    'y',  ')',  '.',  0x00, 0x14, 0x10, 0x00, 0x00,
    0x07, 0x00, 'a',  'u',  't',  'h',  'o',  'r', 
    's',  0x00, 0x00, 0x00, '<',  0x00, 0x00, 0x00,
    0x02, 0x00, 0x00, 0x00, '8',  0x00, 0x00, 0x00,
    ',',  0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
    '(',  0x00, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x04, 0x00, 'n',  'a',  'm',  'e',  0x00, 0x00,
    0x0b, 0x00, 'L',  'u',  'c',  'a',  ' ',  'P', 
    'e',  'n',  'a',  's',  'a',  0x00, 0x00, 0x00,
    0x0c, 0x00, 0x00, 0x00, 0x85, 0x01, 0x00, 0x00,
    0x14, 0x1a, 0x00, 0x00, 0x0b, 0x00, 'm',  'a', 
    'i',  'n',  't',  'a',  'i',  'n',  'e',  'r', 
    's',  0x00, 0x00, 0x00, '<',  0x00, 0x00, 0x00,
    0x02, 0x00, 0x00, 0x00, '8',  0x00, 0x00, 0x00,
    ',',  0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
    '(',  0x00, 0x00, 0x00, 0x1b, 0x03, 0x00, 0x00,
    0x04, 0x00, 'n',  'a',  'm',  'e',  0x00, 0x00,
    0x0b, 0x00, 'L',  'u',  'c',  'a',  ' ',  'P', 
    'e',  'n',  'a',  's',  'a',  0x00, 0x00, 0x00,
    0x0c, 0x00, 0x00, 0x00, 0x85, 0x01, 0x00, 0x00,
    0x94, '#',  0x00, 0x00, 0x0a, 0x00, 'r',  'e', 
    'f',  'e',  'r',  'e',  'n',  'c',  'e',  's', 
    'h',  0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
    'd',  0x00, 0x00, 0x00, 'X',  0x00, 0x00, 0x00,
    0x05, 0x00, 0x00, 0x00, 'P',  0x00, 0x00, 0x00,
    0x1b, 0x03, 0x00, 0x00, 0x04, 0x00, 't',  'e', 
    'x',  't',  0x00, 0x00, 0x0b, 0x00, 'P',  'C', 
    'L',  ' ',  'l',  'i',  'b',  'r',  'a',  'r', 
    'y',  0x00, 0x00, 0x00, 0x9b, 0x06, 0x00, 0x00,
    0x03, 0x00, 'u',  'r',  'l',  0x00, 0x00, 0x00,
    0x17, 0x00, 'h',  't',  't',  'p',  ':',  '/', 
    '/',  'p',  'o',  'i',  'n',  't',  'c',  'l', 
    'o',  'u',  'd',  's',  '.',  'o',  'r',  'g', 
    '/',  0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    '(',  0x00, 0x00, 0x00, 0x85, 0x01, 0x00, 0x00,
    'p',  0x00, 0x00, 0x00, '<',  0x00, 0x00, 0x00,
    0xbc, 0x00, 0x00, 0x00, ' ',  0x00, 0x00, 0x00,
    0x0c, 0x01, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    0x1b, '?',  0x00, 0x00, 0x09, 0x00, 'c',  'l', 
    'a',  's',  's',  'N',  'a',  'm',  'e',  0x00,
    0x06, 0x00, 'q',  'P',  'c',  'l',  'I',  'O', 
    '1',  0x00, 0x00, 0x00, 0x05, 0x00, 'd',  'e', 
    'b',  'u',  'g',  0x00, ':',  0xa0, 0xa0, 0x00,
    0x07, 0x00, 'v',  'e',  'r',  's',  'i',  'o', 
    'n',  0x00, 0x00, 0x00, 0x0c, 0x00, 0x00, 0x00,
    '<',  0x00, 0x00, 0x00, 0xe8, 0x01, 0x00, 0x00,
    0x00, 0x02, 0x00, 0x00, 0x0c, 0x02, 0x00, 0x00
};
#endif // QT_NO_DEBUG

QT_MOC_EXPORT_PLUGIN(qPclIO, qPclIO)

QT_END_MOC_NAMESPACE
